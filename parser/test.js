...
async function getInfo(url) {
  ...
  async function parse(url) {
    try {
      ...
      let q_a;
      try {
        q_a = await getQ_A(details.ASIN, no_pages_query);
      } catch (e) {
        console.log("error Q_A");
      }
      if (q_a) console.log("QA:" + q_a);

      let reviews;
      console.log(details["ASIN:"]);
      console.log(details);
      try {
        reviews = await getReviews(details.ASIN, no_pages_query);
      } catch (e) {
        console.log("error reviews");
      }
      if (reviews) console.log("RE: " + reviews);
...
getInfo("https://www.amazon.co.uk/Resist-Within-Temptation/dp/B07H5VXGBN/");
//getInfo(  "https://www.amazon.co.uk/Razer-BlackWidow-Elite-Illuminated-programmable/dp/B07FTPKLBF/");
//getReviews("B07H5VXGBN", 1).then(data => console.log(data));
